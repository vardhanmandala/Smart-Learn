...
const VideoPlayer = () => {
  ...
  useEffect(() => {
    if (videoId && currentSegment !== undefined) {
      console.log(`🎬 Starting transcription for segment ${currentSegment}...`);
...
const fetchVideoData = async () => {
  ...
  try {
    ...
    console.log(`✅ Video loaded: ${numSegments} segments, currently at segment ${data.progress?.currentSegment || 0}`);

    // ✅ NEW: Fetch transcriptions for chatbot
    if (data.segmentTranscriptions && data.segmentTranscriptions.length > 0) {
      ...
      if (completedTranscriptions) {
        ...
        console.log(`📝 Loaded transcriptions for chatbot: ${completedTranscriptions.length} chars`);
      }
    }
...
// Refresh transcriptions for chatbot (called after segment completes)
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.segmentTranscriptions && data.segmentTranscriptions.length > 0) {
        ...
        if (completedTranscriptions) {
          setAllTranscriptions(completedTranscriptions);
          console.log(`🔄 Refreshed transcriptions: ${completedTranscriptions.length} chars`);
        }
      }
    }
  } catch (error) {
...
const triggerSegmentTranscription = async (segmentNumber) => {
  // ✅ Debug logs FIRST (before any checks)
  console.log(`🔧 DEBUG: triggerSegmentTranscription called`);
  ...
  if (transcribingSegments.has(segmentNumber)) {
    console.log(`⚠️ Segment ${segmentNumber} already being transcribed, skipping duplicate`);
    return;
  }
  ...
  try {
    ...
    console.log(`🎙️ Triggering transcription for segment ${segmentNumber}...`);
    ...
    if (response.ok) {
      const data = await response.json();
      console.log(`✅ Segment ${segmentNumber} transcription ${data.cached ? 'cached' : 'completed'}`);
      return data;
    } else {
      const errorData = await response.json();
      console.error(`❌ Segment transcription failed:`, errorData.message);
    }
  } catch (error) {
  ...
  const onPlayerReady = (event) => {
    ...
    if (segments[currentSegment]) {
      ...
      console.log(`▶️ Starting at segment ${currentSegment}: ${segments[currentSegment].startTime}s`);
    }
  };
    ...
    const interval = setInterval(() => {
      if (playerRef.current && isPlaying) {
        ...
        if (time >= currentSeg.endTime - 2) {
          ...
          setIsPlaying(false);
          console.log(`⏸️ Segment ${currentSegment} ended at ${time.toFixed(1)}s`);
          handleSegmentEnd();
        }
      }
...
const handleSegmentEnd = () => {
  ...
  if (!currentSeg.quizPassed) {
    console.log(`📝 Loading quiz for segment ${currentSegment}...`);
    loadQuiz(); // ✅ Directly call loadQuiz
  } else {
    console.log(`✅ Segment ${currentSegment} already passed, skipping quiz`);
  }
};
...
const loadQuiz = async () => {
  ...
  try {
    ...
    console.log(`📋 Quiz loaded successfully: ${data.questions.length} questions`);
...
const handleQuizSubmit = async () => {
  ...
  try {
    ...
    console.log(`✅ Quiz submitted: Score ${data.score}%, Passed: ${data.passed}, Attempts: ${data.attempts}`);
    ...
    if (data.passed) {
      ...
      // ✅ No preloading - will transcribe when user starts next segment
    }
...
const handleNextSegment = () => {
  if (currentSegment < segments.length - 1) {
    ...
    console.log(`➡️ Moving to segment ${nextSegment}`);
    ...
    // ✅ NEW: Refresh transcriptions for chatbot
  ...
  const goToSegment = (segmentIndex) => {
    ...
    console.log(`🎯 Jumping to segment ${segmentIndex}`);
      ...
      {showTranscription && (
        <div className={styles.transcriptionPanel}>
          <div className={styles.panelHeader}>
            ...
            <button onClick={() => setShowTranscription(false)}>×</button>
          </div>
  ...
  <div className={styles.loading}>
    ...
    <p>Loading quiz questions...</p>
              ...
              Submit Quiz
                ...
                {quizSubmitted && (
                  <div className={`quiz-result ${quizPassed ? 'passed' : 'failed'}`}>
                    ...
                    {quizPassed ? (
                      // ✅ PASSED: Show next segment or complete button
      ...
      {videoData && (
        <Chatbot
          videoId={videoId}
          transcription={allTranscriptions || 'Watch video segments to generate transcriptions. Questions can be answered after transcription completes.'}
