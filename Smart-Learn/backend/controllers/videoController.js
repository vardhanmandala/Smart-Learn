...
const { transcribeYouTubeVideo } = require('../services/whisperService'); // ← ADD THIS
...
const getYouTubeVideoDetails = async (videoId) => {
  try {
    ...
    try {
      ...
      console.log(`   Has Captions: ${details.hasCaption ? 'YES ✅' : 'NO ❌'}`);
      
      return details;
      
    } catch (ytdlError) {
      console.log(`⚠️ ytdl-core failed: ${ytdlError.message}`);
      ...
      try {
        ...
        console.log(`✅ oEmbed API success: ${details.title}`);
        return details;
        
      } catch (oembedError) {
        console.log(`⚠️ oEmbed failed: ${oembedError.message}`);
...
exports.addVideo = async (req, res) => {
  try {
    ...
    // ✅ FIXED: Initialize segmentTranscriptions array with pending segments
    ...
    const video = new Video({
      ...
      progress: 0,
      segmentWatchTime: [],
      segmentTranscriptions: segmentTranscriptions // ✅ CRITICAL: Initialize array
    });
    ...
    console.log(`✅ Video added: ${youtubeId} with ${totalSegments} segments (${Math.floor(videoDetails.duration/60)} mins)`);
    console.log(`📋 Initialized ${segmentTranscriptions.length} segment placeholders`);
...
exports.getVideoById = async (req, res) => {
  try {
    ...
    res.json({
      ...
      // ✅ CRITICAL FIX: Include segmentTranscriptions
...
exports.getTranscription = async (req, res) => {
  try {
    ...
    // ✅ PRIORITY 1: Check if segment transcriptions exist (NEW SYSTEM)
    if (video.segmentTranscriptions && video.segmentTranscriptions.length > 0) {
      console.log('📝 Checking segment-based transcriptions...');
      ...
      if (completedSegments.length > 0) {
        console.log(`✅ Found ${completedSegments.length} completed segment transcriptions`);
...
## 🎯 Segmented Audio Transcription (Whisper AI)
...
${completedSegments.length < video.totalSegments ? `\n⚠️ **Note:** Not all segments have been transcribed yet. Transcription happens as you watch the video.` : '✅ All segments transcribed!'}`;
    ...
    // ✅ PRIORITY 2: Return cached full transcription if exists (OLD SYSTEM - backward compatibility)
    if (video.transcription) {
      console.log('✅ Returning cached full transcription (old system)');
      return res.json({
    ...
    // ✅ PRIORITY 3: Try YouTube Native Transcript (instant, free)
    console.log(`🎥 No transcription found, attempting YouTube native captions...`);
    const videoUrl = `https://www.youtube.com/watch?v=${video.youtubeId}`;

    try {
      console.log('📝 Method 1: Attempting YouTube native transcript...');
      ...
      if (transcriptData && transcriptData.length > 0) {
        ...
        if (transcriptText.length > 100) {
          console.log(`✅ YouTube transcript found: ${transcriptText.length} chars`);

          const formattedTranscript = `# ${video.title}

## 🎯 Video Transcription (YouTube Captions)
...
**Note:** For better segment-specific transcriptions, we now use Whisper AI which transcribes each segment as you watch.`;
          ...
          console.log('✅ Transcription saved (YouTube Native)');
          return res.json({ transcription: formattedTranscript, source: 'youtube' });
        }
      }

    } catch (ytError) {
      console.log(`⚠️ YouTube transcript failed: ${ytError.message}`);
    }

    // ✅ PRIORITY 4: Inform user to use segment-based transcription
    console.log('⚠️ No transcript available - user should watch video for segment transcription');
...
exports.getSegmentTranscription = async (req, res) => {
  try {
    ...
    console.log(`🎯 Segment transcription request: Video ${id}, Segment ${segment}`);
    ...
    // ✅ FIXED: Ensure segmentTranscriptions exists
    if (!video.segmentTranscriptions || video.segmentTranscriptions.length === 0) {
      console.log(`⚠️ Initializing segmentTranscriptions array for video ${id}`);
    ...
    // ✅ NEW: Check current status to prevent duplicates
    ...
    if (segmentIndex >= 0) {
      ...
      if (currentSegment.status === 'completed' && currentSegment.transcription) {
        console.log(`✅ Returning cached segment ${segmentNum} transcription`);
        return res.json({
          segment: segmentNum,
      ...
      if (currentSegment.status === 'processing') {
        console.log(`⚠️ Segment ${segmentNum} already being transcribed, rejecting duplicate request`);
    ...
    console.log(`🎙️ Starting transcription for segment ${segmentNum}...`);
    ...
    // ✅ FIXED: Update using array index
// ✅ FIXED: Update using direct object replacement (forces Mongoose to detect change)
...
if (updateIndex >= 0) {
  ...
  // ✅ VERIFY: Reload from DB to confirm save
  const verifyVideo = await Video.findOne({ _id: id, userId });
  const verifySegment = verifyVideo.segmentTranscriptions[updateIndex];
  
  console.log(`✅ Segment ${segmentNum} transcription saved successfully`);
  console.log(`💾 Data size: ${result.transcription.length} chars, ${(result.audioData.length / 1024).toFixed(2)} KB audio`);
  console.log(`✅ VERIFIED: Status in DB = "${verifySegment.status}"`);
  
  if (verifySegment.status !== 'completed') {
    console.error(`❌ CRITICAL: Status not saved correctly! DB shows: "${verifySegment.status}"`);
  }
} else {
  console.error(`❌ Segment ${segmentNum} not found in array after transcription`);
}


// ✅ Cleanup full audio if this is the last segment
if (segmentNum + 1 >= video.totalSegments) {
  console.log(`🎉 Last segment transcribed! Cleaning up full audio...`);
...
exports.generateQuiz = async (req, res) => {
  try {
    ...
    console.log(`📊 Total segments: ${video.totalSegments}`);
...
if (quiz && quiz.questions.length > 0) {
  console.log(`📋 Returning existing quiz for segment ${segmentIndex}`);
  
  // ✅ NEW: Display correct answers for existing quiz
  console.log(`✅ QUIZ GENERATED - Segment ${segmentIndex}\n`);
  questions.forEach((q, i) => {
    const correctLetter = String.fromCharCode(65 + q.correctAnswer);
    console.log(`   Q${i + 1}: ${q.question.substring(0, 50)}...`);
    console.log(`   ✅ Answer: ${correctLetter}. ${q.options[q.correctAnswer]}\n`);
  });
  ...
  return res.json({ 
    ...
    // ✅ FIXED: Get segment-specific transcription FIRST (before checking old transcription)
    ...
    if (segmentDoc && segmentDoc.transcription) {
      ...
      console.log(`📝 Using segment ${segmentIndex} transcription: ${segmentTranscript.length} chars (from Whisper)`);
    } else {
      console.log(`⚠️ Segment ${segmentIndex} transcription not found, trying fallback...`);
      
      // Fallback: Try to use full video transcription (old method)
      if (video.transcription) {
        console.log(`📝 Using full video transcription as fallback`);
        ...
        console.log(`📝 Extracted ${segmentTranscript.length} chars from full transcript`);

        // If extracted segment is too short, use more context
        if (segmentTranscript.length < 200) {
          console.log('⚠️ Extracted segment too short, using larger portion');
          segmentTranscript = video.transcription.substring(0, 5000);
        }
      } else {
        // No transcription available at all
        console.log('❌ No transcription available (segment or full)');
        return res.status(400).json({
    ...
    // Clean up transcript
    segmentTranscript = segmentTranscript.replace(/\*\*⚠️ TRANSCRIPTION NOTE:\*\*/g, '');
    segmentTranscript = segmentTranscript.replace(/---/g, '');
    segmentTranscript = segmentTranscript.trim();

    // Validate transcript length
    if (segmentTranscript.length < 50) {
      console.log(`❌ Transcript too short: ${segmentTranscript.length} chars`);
      return res.status(400).json({
    ...
    console.log(`✅ Final transcript ready: ${segmentTranscript.length} chars`);
    console.log(`📝 Generating quiz for segment ${segmentIndex}...`);
...
console.log(`\n${'─'.repeat(60)}\n`);
      ...
      console.error('❌ Gemini quiz error:', geminiError.message);
...
exports.submitQuiz = async (req, res) => {
  try {
    ...
    console.log('📥 Quiz submission received for segment', segmentIndex);
    ...
    console.log(`📊 Quiz Result: ${score}% (${correctCount}/${quiz.questions.length}) - ${passed ? 'PASSED' : 'FAILED'}`);
    ...
    if (passed) {
      ...
      console.log(`✅ Progress updated: ${video.progress}% complete`);
    }
...
exports.deleteVideo = async (req, res) => {
  try {
    ...
    console.log(`🗑️ Delete request for video: ${id}`);
    ...
    console.log(`   📦 Audio data freed from database`);
...
exports.handleChat = async (req, res) => {
  try {
    ...
    // ✅ NEW: Combine segment transcriptions for chatbot context
    let videoContext = '';
    ...
    if (video.segmentTranscriptions && video.segmentTranscriptions.length > 0) {
      ...
      if (completedSegments.length > 0) {
        console.log(`🤖 Using ${completedSegments.length} segment transcriptions for chatbot context`);
    ...
    if (!videoContext && video.transcription) {
      console.log('🤖 Using full video transcription for chatbot (fallback)');
...
Provide a helpful, accurate answer based on the video content. If the question is not related to the video, politely redirect them to ask about the video content.`;
    ...
    console.log(`💬 Chatbot response generated (${answer.length} chars)`);
...
exports.chatWithVideo = async (req, res) => {
  try {
    ...
    if (!message || !transcription) {
      return res.status(400).json({ message: 'Message and transcription are required' });
    }

    // Use Gemini AI to answer the question
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });

    const prompt = `You are an AI learning assistant helping a student understand a video lesson.
...
**Instructions:**
- Answer the question based ONLY on the video transcription content
- Be concise and educational
- If the question cannot be answered from the transcription, politely say so
- Use simple, clear language
- Provide examples from the video when relevant
